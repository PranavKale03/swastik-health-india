  ...
  Future<void> sendDataToApi() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final drId = responseData['_id'];

        // Assign companies to the lab technician
        await assignCompaniesToLabTech(drId);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Failed to add Doctor. Error: ${response.statusCode}'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to add Doctor. Exe: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
    ...
    for (var company in _assignedCompanies) {
      ...
      if (response.statusCode != 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'Failed to assign company ${company['company']} to Doctor.'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: CircleAvatar(
        backgroundColor: _generateRandomColor(Doctor.name),
        child: Text(
...
class ManageDoctorDialog extends StatefulWidget {
  final DoctorData Doctor;
  final List<String> availableCompanies;
  final int maxCompanies;

  const ManageDoctorDialog({
    required this.Doctor,
  ...
  @override
  _ManageDoctorDialogState createState() => _ManageDoctorDialogState();
}

class _ManageDoctorDialogState extends State<ManageDoctorDialog> {
  ...
  void initState() {
    super.initState();
    lab_technician_id = widget.Doctor.id;
    ...
    _mobileController = TextEditingController(text: widget.Doctor.mobile);
    ...
    _selectedCompanies = List.from(widget.Doctor.assignedCompanies);
...
class AllDoctorTech extends StatelessWidget {
  const AllDoctorTech({
